...
Conceal      { fg=comment };
Whitespace   { fg=comment };                   -- 'listchars'
...
DiffAdd      { fg=green.da(20), bg=bg };
DiffDelete   { fg=red, bg=bg };
DiffChange   { fg=yellow.da(20), bg=bg };
...
Error      { fg=bg, bg=salmon };             --  any erroneous construct
Todo       { fg=blue };  --  anything that needs extra attention
...
TSVariable           { fg=purple };                 -- Any variable name that does not have another highlight
TSVariableBuiltin    { TSVariable };    -- Variable names that are defined by the languages like `this` or `self`.
...
TSError              { fg=salmon };                -- syntax/parser errors.
